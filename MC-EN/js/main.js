...
$(document).on("click", ".ip", () => {
    ...
    $(".ip").html("<span class'extrapad'>Copied IP!</span>");
    setTimeout(() => {
        $(".ip").html(t);
...
$(document).ready(() => {
    ...
    if (ip == "" || ip == null) return console.error("Request Error - Is the Server IP correct?");
...
const updatePlayerCount = (ip, port) => {
    $.get(`https://api.bybilly.uk/api/players/${ip}/${port}`, (result) => {
        if (result.hasOwnProperty('online')) {
            ...
            $(".playercount").html("Our Server is currently offline!");
        }
    });
}; 